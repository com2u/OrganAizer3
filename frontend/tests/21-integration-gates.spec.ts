...
test('optionale Arbeitsbereiche erscheinen nur nach Konfiguration', async ({ page }) => {
  await page.route('**/api/verbindungen/capabilities', route => route.fulfill({ json: {
    ...
    excalidraw: { added: true, configured: true, public_url: 'https://excalidraw.example.org' },
  ...
  await page.route('**/api/excalidraw/status', route => route.fulfill({ json: { available: true, storage: 'browser' } }))
  ...
  await expect(page.locator('iframe[title="Excalidraw Whiteboard"]')).toHaveAttribute('src', 'https://excalidraw.example.org/workspace-login/excalidraw?ticket=signed-test-ticket')
